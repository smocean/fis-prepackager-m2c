...
Cache.prototype = {
	...
	write: function(key, content) {
		if (arguments.length === 1) {
			...
			this.cacheContent[key] = fis.util.merge(content, fingerprint(key, this.optimize));
		}
	...
	_getCache: function(path) {
		...
		if (cache && cache.timestamp === flag.timestamp && cache.version === flag.version && cache.optimize === flag.optimize) {
			return cache;
		}
